...
WRAPPER_PATH = Path(__file__).resolve().parent.parent / "riscv64-gf-elf-tcf-gcc"
...
class TestTCFWrapper:
    @pytest.mark.parametrize(
        ...
        [
            (
                ...
                [],
                [],
                "/mock/riscv64-gf-elf-gcc -march=rv64imac -mtune=arc-v-rpx-100-series " "-mabi=ilp32 -mcmodel=medany -mno-strict-align -c foo.c",
            ),
            (
                [],
                ["-tcf-no-compressed"],
                [],
                "/mock/riscv64-gf-elf-gcc -march=rv64i_m_a -mtune=arc-v-rpx-100-series " "-mabi=ilp32 -mcmodel=medany -mno-strict-align -c foo.c",
            ),
            (
                [],
                ["-tcf-with-memory-defines"],
                [],
                "/mock/riscv64-gf-elf-gcc -march=rv64imac -mtune=arc-v-rpx-100-series "
            ...
            (
                ...
                ["-tcf-with-memory-defines"],
                ["-O2", "-I/usr/include"],
                "/mock/riscv64-gf-elf-gcc -v -Wall -march=rv64imac -mtune=arc-v-rpx-100-series "
                "-mabi=ilp32 -mcmodel=medany -mno-strict-align "
                "-Wl,-defsym=txtmem_addr=0x0 -Wl,-defsym=__flash=0x0 "
                "-Wl,-defsym=txtmem_len=0x20000 -Wl,-defsym=__flash_size=0x20000 "
                "-Wl,-defsym=datamem_addr=0x200000 -Wl,-defsym=__ram=0x200000 "
    ...
    def test_arguments(self, capsys, args_before_tcf, extra_tcf_flags, args_after_tcf, expected):
        ...
        mock_compiler_info.get_compiler_path.return_value = "/mock/riscv64-gf-elf-gcc"
        mock_compiler_info.get_compiler_triplet.return_value = "riscv64-gf-elf"
